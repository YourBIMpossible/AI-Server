...
| 4 | Model choice is a config line backed by an eval score | **Met for rig clients; box-side gap** | The pick `gemma4:26b-a4b-it-q4_K_M` (27/27) is in `config/models.txt`. The rig's `.env` sets `INFERENCE_BASE_URL=http://192.168.1.128:11434/v1` and `INFERENCE_MODEL=gemma4:26b-a4b-it-q4_K_M`, so rig clients use the pick. The box has no `~/AI-Server/.env`, so boot preload and box-run jobs fall back to `qwen2.5-coder:14b`. |
...
1. **Criterion 4, box side:** so the box's boot preload and box-run jobs use the pick as well, run this on the box: `printf 'INFERENCE_MODEL=gemma4:26b-a4b-it-q4_K_M\n' > ~/AI-Server/.env && chmod 600 ~/AI-Server/.env`, Then run `python3 ~/AI-Server/scripts/preload.py`.
